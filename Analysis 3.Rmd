...
DELHI.TEST=as.tibble(read.csv("DailyDelhiClimateTestEdited.csv"))              #DO NOT CHANGE
...
clean.func = function(data){
  data <- data %>%
    ...
    rename("Temperature"="meantemp","Humidity"="humidity","Wind Speed"="wind_speed","Pressure"="meanpressure")   
  return(data)}                                                               
...
TRAIN.CLEAN.2$Month = as.numeric(TRAIN.CLEAN.2$Month)
...
TEST.CLEAN.3$Day  = rep(c("Tuesday", "Wednesday", "Thursday","Friday","Saturday","Sunday","Monday"),length.out=length(TEST.CLEAN.3$Day))
...
avgTemps = TRAIN.FINAL %>%
  group_by(Month,Day) %>%
  summarise(mean = mean(Temperature))

for(j in 1:12){    #DO NOT CHANGE
  for(k in 1:7){   #DO NOT CHANGE
    if ((j = avgTemps$Month[j]) & (k = TRAIN.CLEAN$Day[k])){
      MEAN.TEMP.TRAIN[j,k] = mean(TRAIN.CLEAN$Temperature)}}}   #DO NOT CHANGE
