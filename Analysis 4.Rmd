...
DATA2<-DATA %>% na.omit() %>% crossv_kfold(10)
...
```
In the code chunk below, we begin by initiating an empty $7 \times 7$ matrix of missing values called `OUT.RMSE`. Each row corresponds to a different choice of $I$ and each column corresponds to a different choice of $J$. 

Apply the code in the previous [Tutorial](https://supermariogiacomazzo.github.io/STOR320_WEBSITE/Tutorial/Modeling%204/Tutorial.html) 
in a double loop that 
performs 10-Fold CV to estimate out-of-sample RMSE under each polynomial model where $I \in \{1,2,3, \cdots,7\}$ and $J \in \{1,2,3, \cdots,7\}$ and then saves the RMSE in the $(I,J)$-cell of the matrix `OUT.RMSE`.

Code and Output:
```{r,eval=T}
OUT.RMSE=matrix(NA,7,7)#DO NOT CHANGE

for(i in 1:nrow(OUT.RMSE)) {
  for(j in 1:ncol(OUT.RMSE)) {
    DATA3=DATA2 %>% 
    ...
    DATA3.PREDICT = DATA3 %>% 
          ...
          select(predict) %>%
          unnest()
    OUT.RMSE[i,j] <- RMSE.func(actual=DATA3.PREDICT$W,predict=DATA3.PREDICT$.fitted)
  }
}
...
Code and Output:
```{r,eval=T}
BEST5.RMSE <- as_data_frame(OUT.RMSE2)

BEST5.RMSE <- BEST5.RMSE[order(BEST5.RMSE$RMSE),]
...
BEST5.DATA2 = BEST5.DATA %>%
  gather(key="Model", value="Predict", factor_key = T, 6:10)
...
```{r,eval=T}
ggplot(BEST5.DATA2) + geom_point(aes(x=D,y=W),alpha=0.05,stroke=0) + theme_minimal() + xlab("Julian Day")+ ylab("Max Water Temperature") + geom_line(aes(x=D,y=Predict, color=Model))
```

Answer *(2 Points)*: For each of the top five models, the max temperatures look pretty similar based on the graph above.
...
Code and Output:
```{r,eval=T}
model1 <- lm(W ~ poly(A,3) + poly(D, 6), data= DATA)
resids <- resid(model1)
DATA5 <- DATA %>%
  mutate(predictions = predict(model1), resids2 = residuals(model1))

ggplot(DATA5) +
  geom_point(aes(x=W, y=predictions)) +
...
```{r,eval=T}
DATA %>%
  mutate(bestmodresid = model1$residuals) %>%
  ggplot(aes(x = D, y = bestmodresid)) +
  geom_point() +
